    ...
    return eventData.items.map( event => (
        {
            ...
            presaleEnabled: Boolean(event.presaleEnabled),
            presalePassword: event.presalePassword ?? null,
            presaleEndsAt: event.presaleEndsAt ? new Date(event.presaleEndsAt) : null,
            price: event.price,
